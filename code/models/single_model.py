...
class SingleModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            if opt.continue_train:
                opt.lr = opt.lr - opt.which_epoch*opt.lr/opt.niter_decay
    ...
    def predict(self):
        with torch.no_grad():
            ...
            # self.fake_B=self.trans.PHVIT(self.fake_B)
    ...
    def forward(self):
        ...
        self.real_A_gray = Variable(self.input_A_gray)
        self.real_img = Variable(self.input_img)
        # self.hvi_real_B = Variable(self.trans.HVIT(self.real_B))
        # print(self.real_img.max())
        ...
        if self.opt.skip == 1:
            self.fake_B, self.latent_real_A,self.k = self.netG_A.forward(self.real_img, self.real_A_gray,get_k=True)
        else:
            self.fake_B = self.netG_A.forward(self.real_img, self.real_A_gray)
    ...
    def backward_G(self, epoch):
        ...
        self.loss_G_A = self.loss_G_A*8*F.mse_loss(self.fake_B, self.real_B)
        ...
        if self.opt.vgg > 0:
            ...
            # self.loss_G = self.loss_vgg_b * vgg_w
        elif self.opt.fcn > 0:
