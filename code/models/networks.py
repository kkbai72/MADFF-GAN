...
from models.selfmodule import MDECM,Chosenfuse,LayerNorm2d
###############################################################################
# Functions
###############################################################################
...
class Unet_resize_conv(nn.Module):
    def __init__(self, opt, skip):
        ...
        self.compose = transforms.Compose([transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
        self.transform = self.compose
        self.k = torch.nn.Parameter(torch.full([1], 2.))
        # self.k =2.45
        self.opt = opt
        self.skip = skip
        p = 1
        ...
        if self.opt.use_norm == 1:
            self.bn1_1 = SynBN2d(32) if self.opt.syn_norm else LayerNorm2d(32)
        # self.conv1_2 = nn.Conv2d(32, 32, 3, padding=p)
        self.conv1_2 = MDECM(32,32,3)
        self.LReLU1_2 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn1_2 = SynBN2d(32) if self.opt.syn_norm else LayerNorm2d(32)
        self.max_pool1 = nn.AvgPool2d(2) if self.opt.use_avgpool == 1 else nn.MaxPool2d(2)

        self.conv2_1 = nn.Conv2d(32, 64, 3, padding=p)
        self.LReLU2_1 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn2_1 = SynBN2d(64) if self.opt.syn_norm else LayerNorm2d(64)
        # self.conv2_2 = nn.Conv2d(64, 64, 3, padding=p)
        self.conv2_2 = MDECM(64,64,3)
        self.LReLU2_2 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn2_2 = SynBN2d(64) if self.opt.syn_norm else LayerNorm2d(64)
        self.max_pool2 = nn.AvgPool2d(2) if self.opt.use_avgpool == 1 else nn.MaxPool2d(2)

        self.conv3_1 = nn.Conv2d(64, 128, 3, padding=p)
        self.LReLU3_1 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn3_1 = SynBN2d(128) if self.opt.syn_norm else LayerNorm2d(128)
        # self.conv3_2 = nn.Conv2d(128, 128, 3, padding=p)
        self.conv3_2 = MDECM(128,128,3)
        self.LReLU3_2 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn3_2 = SynBN2d(128) if self.opt.syn_norm else LayerNorm2d(128)
        ...
        if self.opt.use_norm == 1:
            self.bn4_1 = SynBN2d(256) if self.opt.syn_norm else LayerNorm2d(256)
        # self.conv4_2 = nn.Conv2d(256, 256, 3, padding=p)
        self.conv4_2 = MDECM(256,256,3)
        self.LReLU4_2 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn4_2 = SynBN2d(256) if self.opt.syn_norm else LayerNorm2d(256)
        self.max_pool4 = nn.AvgPool2d(2) if self.opt.use_avgpool == 1 else nn.MaxPool2d(2)

        self.conv5_1 = nn.Conv2d(256, 512, 3, padding=p)
        self.LReLU5_1 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn5_1 = SynBN2d(512) if self.opt.syn_norm else LayerNorm2d(512)
        self.conv5_2 = MDECM(512, 512, 3)
        self.LReLU5_2 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn5_2 = SynBN2d(512) if self.opt.syn_norm else LayerNorm2d(512)

        self.deconv5 = nn.Conv2d(512, 256, 3, padding=p)
        self.conv6_1 = Chosenfuse(512,256)
        # self.conv6_1 = nn.Conv2d(512, 256, 3, padding=p)
        self.LReLU6_1 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn6_1 = SynBN2d(256) if self.opt.syn_norm else LayerNorm2d(256)
        self.conv6_2 = nn.Conv2d(256, 256, 3, padding=p)
        self.LReLU6_2 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn6_2 = SynBN2d(256) if self.opt.syn_norm else LayerNorm2d(256)

        self.deconv6 = nn.Conv2d(256, 128, 3, padding=p)
        self.conv7_1 = Chosenfuse(256,128)
        # self.conv7_1 = nn.Conv2d(256, 128, 3, padding=p)
        self.LReLU7_1 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn7_1 = SynBN2d(128) if self.opt.syn_norm else LayerNorm2d(128)
        self.conv7_2 = nn.Conv2d(128, 128, 3, padding=p)
        self.LReLU7_2 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn7_2 = SynBN2d(128) if self.opt.syn_norm else LayerNorm2d(128)

        self.deconv7 = nn.Conv2d(128, 64, 3, padding=p)
        self.conv8_1 = Chosenfuse(128,64)
        # self.conv8_1 = nn.Conv2d(128, 64, 3, padding=p)
        self.LReLU8_1 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn8_1 = SynBN2d(64) if self.opt.syn_norm else LayerNorm2d(64)
        self.conv8_2 = nn.Conv2d(64, 64, 3, padding=p)
        self.LReLU8_2 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn8_2 = SynBN2d(64) if self.opt.syn_norm else LayerNorm2d(64)

        # self.deconv8 = nn.ConvTranspose2d(64, 32, 2, stride=2)
        self.deconv8 = nn.Conv2d(64, 32, 3, padding=p)
        self.conv9_1 = Chosenfuse(64,32)
        # self.conv9_1 = nn.Conv2d(64, 32, 3, padding=p)
        self.LReLU9_1 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn9_1 = SynBN2d(32) if self.opt.syn_norm else LayerNorm2d(32)
    ...
    def forward(self, input, gray,get_k=False):
        flag = 0
        ...
        if self.opt.use_norm == 1:
            if self.opt.self_attention:
                ...
                x = self.bn1_1(self.LReLU1_1(self.conv1_1(input)))
            x=self.conv1_2(x,gray)
            conv1 = self.bn1_2(self.LReLU1_2(x))
            x = self.max_pool1(conv1)

            x = self.bn2_1(self.LReLU2_1(self.conv2_1(x)))
            x = self.conv2_2(x,gray_2)
            conv2 = self.bn2_2(self.LReLU2_2(x))
            x = self.max_pool2(conv2)

            x = self.bn3_1(self.LReLU3_1(self.conv3_1(x)))
            x = self.conv3_2(x,gray_3)
            conv3 = self.bn3_2(self.LReLU3_2(x))
            x = self.max_pool3(conv3)

            x = self.bn4_1(self.LReLU4_1(self.conv4_1(x)))
            x = self.conv4_2(x,gray_4)
            conv4 = self.bn4_2(self.LReLU4_2(x))
            x = self.max_pool4(conv4)

            x = self.bn5_1(self.LReLU5_1(self.conv5_1(x)))
            # x = x*gray_5 if self.opt.self_attention else x
            x = self.conv5_2(x,gray_5)
            conv5 = self.bn5_2(self.LReLU5_2(x))
            ...
            x = self.bn6_1(self.LReLU6_1(self.conv6_1(conv5, conv4)))
            ...
            x = self.bn7_1(self.LReLU7_1(self.conv7_1(conv6, conv3)))
            ...
            x = self.bn8_1(self.LReLU8_1(self.conv8_1(conv7, conv2)))
            ...
            x =  self.bn9_1(self.LReLU9_1(self.conv9_1(conv8, conv1)))
            ...
            # output = self.depth_to_space(conv10, 2)k
            if self.opt.tanh:
                latent = self.tanh(latent)
            if self.skip:
                # if self.opt.linear_add:
                    # if self.opt.latent_threshold:9
        ...
        output = (output+1)/2.0
        # output = self.trans.PHVIT(output)
        if self.skip:
            if get_k:
                return output,latent,self.k
